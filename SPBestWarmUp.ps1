...
Function Installer() {
	...
	# Create Task
	schtasks /create /tn "SPBestWarmUp" /ru $user /rp $pass /rl highest /sc daily /st 04:00 /ri 60 /du 24:00 /tr "PowerShell.exe -ExecutionPolicy Bypass $global:path"
...
Function WarmUp() {
	...
	$was |? {$_.IsAdministrationWebApplication -eq $true} |% {$caUrl = Get-SPWeb $_.Url }
	...
	$hnsc = Get-SPSite -Limit All | select Url
	foreach ($sc in $hnsc) {
		IENavigateTo $sc.Url
	}
